...
public class InspectorView extends ViewPart {
   ...
   protected void fillToolBar(Composite parent) {
      ...
      backButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.BACKWARD_NAV));
      ...
      foreButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.FORWARD_NAV));
   ...
   public void populate(String title, String type, LispNode content) {
      ...
      if ((content.params.size() > 0) && (content.params.get(0) != null) &&
         ...
         for (LispNode item : content.params.get(0).params) {
            if (item.isString) {
               ...
               output.appendInspectable(item.getf(":value").value,
                                        item.get(2).value);
            }
         }
      }
