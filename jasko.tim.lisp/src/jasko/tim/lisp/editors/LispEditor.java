...
public class LispEditor extends TextEditor implements ILispEditor {
   ...
   public static final String                          EDITOR_CONTEXT          = EDITOR_ID +
                                                                                        ".context";
   /** The ID of the editor ruler context menu */
   public static final String                          RULER_CONTEXT           = EDITOR_CONTEXT +
                                                                                        ".ruler";
   ...
   private final String                                CHANGED_POS_FOR_COMPILE = "jasko.tim.lisp.doc.change_for_compile";
   private boolean                                     useAutoBuild            = false;

   private final String                                CHANGED_POS_FOR_OUTLINE = "jasko.tim.lisp.doc.change_for_outline";

   private final String                                TOP_LVL_POS             = "jasko.tim.lisp.doc.top_lvl_pos";
   ...
   /**
    * Jumps the user to a given position in the given file. If an editor is not
    * open for this file, one is opened. If the file is not in the workspace, a
    * link is created to it. If the position is out of bound, we try to find the
    * snippet. If we can't find that, at least they have the file open.
    * 
    * @param filePath
    * @param position
    * @param snippet
    */
   public static void jumpToDefinition(String filePath,
                                       int position,
                                       String snippet) {
    ...
    * open for this file, one is opened. If the file is not in the workspace, a
    * link is created to it. If the position is out of bound, we try to find the
    * snippet. If the snippet isn't found, we go to the first instance of
    * symbol. If we can't find that, at least they have the file open.
    * 
            ...
            if (symbol == null) {
               ...
               while (offset >= 0 &&
                        ...
                        (doc.getPartition(offset)
                            .getType()
                            .equals(LispPartitionScanner.LISP_COMMENT) || doc.getPartition(offset)
                                                                             .getType()
                                                                             .equals(LispPartitionScanner.LISP_STRING))) {
   ...
   public void createPartControl(Composite parent) {
      ...
      if (null != doc) {
         ...
         useAutoBuild = LispPlugin.getDefault()
                                  .getPreferenceStore()
   ...
   protected ISourceViewer createSourceViewer(Composite parent,
                                              ...
                                              int styles) {
      SourceViewer viewer = new ProjectionViewer(parent,
                                                 ruler,
                                                 getOverviewRuler(),
   ...
   public void showPopupInfo(String message) {
      ...
      Point p = txt.getLocationAtOffset(this.getSourceViewer()
                                            .getSelectedRange().x);
   ...
   private void processAutoBuild() {
      ...
      if (useAutoBuild != oldAutoBuild || !useAutoBuild) { // autobuild is on
                                                           // and changed
   ...
   private int[][] getFormsToCompile(Position[] pos,
      ...
      for (Position p : pos) {
         for (int i = p.offset; i <= p.offset + p.length; ++i) {
            if (i < range[0] || i > range[0] + range[1]) { // not in previous
                                                           // range
                        ...
                        if (range == null) { // not in sexp
                           ...
                           try {
                              ...
                              if (end > start) {
                                 range[0] = start;
                                 range[1] = end - start + 1;
                                 LispMarkers.deleteMarkers(getIFile(),
                                                           range[0],
                                                           range[1]);
                              }
                              else {
                           ...
                           if ( !sexpOffsets.contains(range[0])) {
                              sexpOffsets.add(new Integer(range[0]));
                              sexpOffsetsEnd.add(new Integer(range[0] +
                                       range[1]));
                           }
                     }
                  }
      ...
      // and one of them is modified, add compilation of all forms after
      // modified
      ...
      for (TopLevelItem itm : newForms) {
         ...
         if (itm.type.equalsIgnoreCase("in-package")) {
            ...
            if (multForms.contains(itmTmp)) {// duplicate items of modified
                                             // forms
               sexpOffsets.add(new Integer(itm.offset));
               sexpOffsetsEnd.add(new Integer(itm.offsetEnd));
            }
            else
               if (sexpOffsets.contains(new Integer(itm.offset))) { // modified
                                                                    // form
   ...
   // undefines removed forms and returns unchanged forms that still should be
   // defined
   ...
   private ArrayList<TopLevelItem> undefineRemovedForms(ArrayList<TopLevelItem> newForms) {
      ...
      for (TopLevelItem item : topForms) {
         ...
         if (itemTmp.toLowerCase().startsWith("in-package")) {
            ...
            if ( !newTypeNamePkg.contains(itemTmp)) { // if not in new forms -
                                                      // set to undefine
               ...
               if (notToUndefine.contains(itemTmp)) {
                  // it it is already in toDefine list - it was already
                  // processed
                  if ( !toDefineNoPos.contains(itemTmp)) {
                     ...
                     for (String itm : newTypeNamePkg) {
                        if (itm.equalsIgnoreCase(item.type + "," + item.name +
                                 "," + item.pkg)) {
                           ++nnew;
                        }
                     }
   ...
   public void updateFoldingStructure(HashSet<Position> positions,
      ...
      HashMap<Position, ProjectionAnnotation> hashAnnotations = new HashMap<Position, ProjectionAnnotation>();
      ...
      projectionAnnotationModel.modifyAnnotations(remAnnotations,
                                                  newAnnotations,
                                                  null);
