...
public class LispOutlinePage extends ContentOutlinePage implements
         MouseListener, KeyListener, MouseTrackListener {
   ...
   private HashMap<TopLevelItem, Position> itemPos = new HashMap<TopLevelItem, Position>();
   private HashMap<TopLevelItem, TreeItem> itemTr  = new HashMap<TopLevelItem, TreeItem>();
   ...
   private void updateTree() {
      ...
      for (Position pos : changedPos) {
         if ( !pos.isDeleted) {
            ...
            for (int i = 0; i < pos.length; ++i) {
               offset = new Integer(LispUtil.getTopLevelOffset(doc,
                                                               Math.min(pos.offset +
                                                                                 i,
                                                                        doc.getLength() - 1),
                                                               true));
         ...
         if (modifiedPos.contains(new Integer(item.offset))) {
            ...
            try {
               if (doc.getChar(item.offset) == ';') { // section
                  String val = doc.get(item.offset,
                                       doc.getLineLength(doc.getLineOfOffset(item.offset)));
                  itm = LispUtil.getSectionItem(new LispComment(val,
                                                                item.offset,
                                                                item.offset +
                                                                         val.length()),
                                                item.offset);
                  ...
                  LispNode exp = LispParser.parse(LispUtil.getTopLevelExpression(doc,
            ...
            if (itm != null) {
               ...
               if (sort == Sort.Type && !item.type.equals(itm.type)) {
                  TreeItem[] typeNodes = getTreeViewer().getTree().getItems();
                  int i, j;
                  ...
                  if (i >= typeNodes.length) { // create new category
                     typeNode = new TreeItem(getTreeViewer().getTree(),
                                             SWT.NONE);
                     typeNode.setText(itm.type);
                     typeNode.setImage(CuspResources.getImageForType(itm.type));
                  }
                  else {
                     typeNode = typeNodes[i];
                  }
                  TreeItem tmp = new TreeItem(typeNode,
                                              SWT.NONE,
                  ...
                  if (sort == Sort.Alpha && !item.name.equals(itm.name)) {
                     TreeItem[] nodes = getTreeViewer().getTree().getItems();
                     int i;
                     for (i = 0; i < nodes.length; ++i) {
                        if (nodes[i].getText().compareToIgnoreCase(itm.name) >= 0) {
                           break;
                        }
                     }
                     TreeItem tmp = new TreeItem(getTreeViewer().getTree(),
                                                 SWT.NONE,
                                                 i);
                     tr.setText(itm.name);
                     copyItem(tr, tmp);
      ...
      ArrayList<TopLevelItem> newItems = new ArrayList<TopLevelItem>(changedTopLevelPos.size());
      ...
      if (sort == Sort.Position) {
         ...
         for (TopLevelItem item : newItems) {
            ...
            TreeItem tmp = null;
            if (i > 0 &&
                     ((TopLevelItem) (roots[i - 1].getData())).type.equals("section")) {
               ...
               if (item.type.equals("section")) {
                  ...
                  for (int j = i; j < roots.length; ++j) {
                     if (((TopLevelItem) (roots[j].getData())).type.equals("section")) {
                        break;
                     }
                     else {
                        TreeItem tmpkid = new TreeItem(tmp, SWT.NONE);
         ...
         if (sort == Sort.Type) {
            for (TopLevelItem item : newItems) {
               ...
               TreeItem typeNode;
               if (i >= typeNodes.length) { // create new category
                  typeNode = new TreeItem(getTreeViewer().getTree(), SWT.NONE);
                  typeNode.setText(item.type);
                  typeNode.setImage(CuspResources.getImageForType(item.type));
               }
               else {
                  typeNode = typeNodes[i];
               }
   ...
   private int getIndex(int offset, TreeItem[] items) {
      ...
      if (offset > item0.offset) {
         for (i = 1; i < items.length; ++i) {
            ...
            if (offset > item0.offset && offset < item1.offset) {
               break;
            }
            item0 = item1;
         }
   ...
   private boolean isDeletedTopLevel(TopLevelItem item) {
      ...
      int offset = LispUtil.getTopLevelOffset(editor.getDocument(),
                                              pos.offset,
                                              true);
   ...
   // works on tree up to 2 levels deep (exactly what we use for
   // LispOutlinePage)
   ...
   public void makeContributions(IMenuManager menuMgr,
      ...
      sortAlpha.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.SORT_ALPHA));
      if (sort == Sort.Alpha) {
      ...
      sortType.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.SORT_TYPE));
      if (sort == Sort.Type) {
      ...
      sortPosition.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.SORT_POSITION));
      if (sort == Sort.Position) {
   ...
   private boolean isSearchable(char c) {
      if ("1234567890qwertyuiopasdfghjklzxcvbnm!@#$%^&*()_-=+{}|[]\\:;\"\'<>?,./`~".indexOf(Character.toLowerCase(c)) >= 0) {
         return true;
      }
      else {
         return false;
   ...
   public void mouseHover(MouseEvent e) {
      ...
      if (showToolTip) {
         ...
         if (item != null && item != tooltipItem && item.getData() != null) {
            final Point ptHint = new Point(e.x, item.getBounds().y +
   ...
   private void cookToolTip(final Tree tree,
                            final Point ptHint,
                            final TopLevelItem tr,
                            final SwankInterface swank,
                            final String variable) {
      swank.sendGetArglist(variable,
                           tr.pkg,
                           new SwankRunnable() {
                              public void run() {
                                 final String args = getReturn().value;
                                 swank.sendGetDocumentation(variable,
                                                            tr.pkg,
                                                            new SwankRunnable() {
                                                               public void run() {
