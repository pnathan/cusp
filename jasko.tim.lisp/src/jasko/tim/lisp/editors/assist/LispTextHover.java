...
public class LispTextHover implements ITextHover, ITextHoverExtension {
   ...
   public String getHoverInfo(ITextViewer textViewer, IRegion hoverRegion) {
      ...
      String function = LispUtil.getCurrentFullWord(doc,
                                                    hoverRegion.getOffset());
      int[] range = LispUtil.getCurrentFullWordRange(doc,
                                                     hoverRegion.getOffset());
      if (function.equals("")) {
         return null;
      }
      else
         if (function.equals(prev) && range[0] == prevOffset) {
            return prevResult;
         }
      ...
      try {
         ...
         e.printStackTrace();
         return "no swank available";
      }
      String result = "";
      if (editor != null) {
         result = swank.getArglist(function,
                                   3000,
                                   LispUtil.getPackage(textViewer.getDocument()
                                                                 .get(),
                                                       hoverRegion.getOffset()));
      }
      else {
