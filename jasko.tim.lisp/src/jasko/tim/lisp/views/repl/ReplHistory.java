...
public class ReplHistory extends SourceViewer implements MouseMoveListener,
   ...
   public ReplHistory(Composite parent) {
      super(parent, new VerticalRuler(10), SWT.V_SCROLL | SWT.MULTI | SWT.LEFT |
               SWT.BORDER);

      setEditable(false);
      // configure(new LispConfiguration(null,
      // LispPlugin.getDefault().getColorManager()));
      ...
      ps.addPropertyChangeListener(new IPropertyChangeListener() {
         public void propertyChange(PropertyChangeEvent event) {
            if (event.getProperty()
                     .equals(PreferenceConstants.DECORATE_REPL_INSPECTABLES)) {
               applyInspectableStyles = (Boolean) event.getNewValue();
            }
            else
               if (event.getProperty()
                        .equals(PreferenceConstants.REPL_INSPECTABLE_UNDERLINE)) {
            ...
            // should refresh already-displayed inspectables, but not sure how
            // to get the currently-applied
            // StyleRanges back to the "normal" syntax highlighted baseline so
            // that the new styles could be applied
            // as necessary
         }
      });
      LispPlugin.getDefault()
                .getColorManager()
                .addChangeEventListener(new ChangeEventListener() {
                   public void colorPreferenceChanged(ColorChangeEvent event) {
                      if (event.tokenType.equals(ColorManager.TokenType.REPL_INSP_BG)) {
                         ...
                         if (event.tokenType.equals(ColorManager.TokenType.REPL_INSP_FG)) {
                            inspectableFore = event.newValue;
                         }
                         else
                            if (event.tokenType.equals(ColorManager.TokenType.REPL_INP_FG)) {
                               inputFore = event.newValue;
                            }
                            else
                               if (event.tokenType.equals(ColorManager.TokenType.REPL_INP_BG)) {
                                  inputBack = event.newValue;
                               }
                               else
                                  if (event.tokenType.equals(ColorManager.TokenType.COMMENT)) {
                      ...
                      // should refresh already-displayed inspectables, but not
                      // sure how to get the currently-applied
                      // StyleRanges back to the "normal" syntax highlighted
                      // baseline so that the new styles could be applied
                      // as necessary
                   }
                });
   }
   ...
   private void applyInspectableStyle(int start, int length) {
      if ( !applyInspectableStyles)
         return;

      StyleRange style = new StyleRange(start,
                                        length,
                                        inspectableFore,
                                        inspectableBack);
   ...
   /*
    * TODO: private void applyInspectableHoverStyle (int start, int length) { //
    * not currently used -- might be nice to allow users to define a :hover
    * style if (!applyInspectableStyles) return; StyleRange style = new
    * StyleRange(start, length, inspectableFore, inspectableBack);
    * style.underline = underlineInspectables;
    * getTextWidget().setStyleRange(style); }
    */
   ...
   protected void showInspectableMenu(final InspectableRegion region) {
      ...
      copyRepl.addSelectionListener(new SelectionListener() {
         ...
         public void widgetSelected(SelectionEvent e) {
            ReplView.getInstance()
                    .insertInInputArea("#.(swank:get-repl-result #10r" +
                             region.id + ")");
         }
      });
