...
/*
 * TODO: Notes on future Repl refactoring Goal - factor out repl. Repl compound
 * (ReplEditor in, and ReplHistory history), should be substitutable by any
 * class (so create Interface) which implements the following functions (some of
 * these eventually should be moved inside repl and handled there): - void
 * clearHistory() to use in places where history.clear() is now used - void
 * clearRepl() to use in places where in is cleared - void appendText(String
 * msg) to append msg to repl - void appendInspectable(String text, String id)
 * also should call swank.sendInspectInspectedPart(partNum, new
 * InspectorRunnable()), when inspectable is selected - void scrollDown() -
 * ILispEditor getILispEditor() - to install LispConfiguration - void
 * setFont(Font font) - String get() get string that was edited in Repl (to send
 * for evaluation) - void selectAll()
 */
...
public class ReplView extends ViewPart implements SelectionListener {
   ...
   protected void setPackageString(String pkg) {
      if (swank != null) {
         ...
         slm.setMessage("CL: " + swank.getLispVersion() + " | Cusp: " +
   ...
   protected SwankInterface getSwank() throws SwankNotFoundException {
   ...
   private class ReplEditor extends SourceViewer implements ILispEditor {
      private final LispConfiguration config = new LispConfiguration(this,
   ...
   public void createPartControl(Composite parent) {
      ...
      fd.setHeight(LispPlugin.getDefault()
                             .getPreferenceStore()
      ...
      in = new ReplEditor(comp2, new VerticalRuler(10), SWT.V_SCROLL |
               SWT.MULTI | SWT.LEFT | SWT.BORDER);
      inconfig = in.config;
      inReadMode = false;
      ...
      IHandlerService keys = (IHandlerService) this.getSite()
                                                   .getService(IHandlerService.class);
      IContextService keyctx = (IContextService) this.getSite()
                                                     .getService(IContextService.class);

      keyctx.activateContext("jasko.tim.lisp.context1");

      HandlerDef[] handler_defs = {
               new HandlerDef(HyperSpecAction.class,
                              "jasko.tim.lisp.actions.HyperSpecAction",
                              true),
               new HandlerDef(LispDocAction.class,
                              "jasko.tim.lisp.actions.LispDocAction",
                              true),
               new HandlerDef(OpenHistoryAction.class,
                              "jasko.tim.lisp.actions.OpenHistoryDialogAction",
                              false),
               new HandlerDef(PreviousREPLCommandAction.class,
                              "jasko.tim.lisp.actions.PreviousREPLCommandAction",
                              false),
               new HandlerDef(NextREPLCommandAction.class,
                              "jasko.tim.lisp.actions.NextREPLCommandAction",
                              false),
               new HandlerDef(ContentAssistAction.class,
                              "jasko.tim.lisp.editors.actions.ContentAssistAction",
                              true),
               new HandlerDef(IndentAction.class,
                              "jasko.tim.lisp.editors.actions.IndentAction",
                              true),
               new HandlerDef(ExpandSelectionAction.class,
                              "jasko.tim.lisp.editors.actions.ExpandSelectionAction",
                              true),
               new HandlerDef(JumpForwardAction.class,
                              "jasko.tim.lisp.editors.actions.JumpForwardAction",
                              true),
               new HandlerDef(JumpBackAction.class,
                              "jasko.tim.lisp.editors.actions.JumpBackAction",
                              true),
               new HandlerDef(FindDefinitionAction.class,
                              "jasko.tim.lisp.editors.actions.FindDefinitionAction",
                              true),
               new HandlerDef(CommentingAction.class,
                              "jasko.tim.lisp.editors.actions.CommentingAction",
                              true) };
      ...
      /*
       * if (swank != null ) { setPackageString(swank.getCurrPackage());
       * getSwank().sendEval("(format nil \"You are running ~a ~a via Cusp " +
       * LispPlugin.getVersion() +
       * "\" (lisp-implementation-type) (lisp-implementation-version))\n", new
       * ReturnHandler()); // appendText("You are running " +
       * swank.getLispVersion() + // " via Cusp " +
       * LispPlugin.getVersion()+"\n");
       * setPackageString(swank.getCurrPackage()); }
       */
       ...
       * PaintObjectListener() { public void paintObject(PaintObjectEvent event)
       * { StyleRange style = event.style; int start = style.start; for (int i =
       * 0; i < offsets.size(); i++) { int offset = offsets.get(i); if (start ==
       * offset) { Point pt = controls.get(i).getSize(); Point loc =
   ...
   protected void fillMenu(Composite parent) {
      IMenuManager menuMgr = this.getViewSite()
                                 .getActionBars()
                                 .getMenuManager();
      menuMgr.add(new Action("Increase Text Size") {
         public void run() {
            ...
            LispPlugin.getDefault()
                      .getPreferenceStore()
                      .setValue(PreferenceConstants.REPL_FONT_SIZE,
                                fd.getHeight());
            Font newFont = new Font(Display.getDefault(), fd);
   ...
   protected void fillNormalToolBar() {
      if (clearButton != null) {
         IToolBarManager tbm = this.getViewSite()
                                   .getActionBars()
                                   .getToolBarManager();
   ...
   protected void fillToolBar(Composite parent) {
      loadPackageButton = new Action("Load Package") {
         public void run() {
            swank.sendGetInstalledPackages(new SwankRunnable() {
               public void run() {
                  final LispNode installedPkgsWithInfo = LispParser.parse(result.getf(":return")
                                                                                .getf(":ok")
                                                                                .get(1).value);
                  ...
                  swank.sendGetAvailablePackages(new SwankRunnable() {
                     public void run() {
                        ...
                        PackageDialog pd = new PackageDialog(ReplView.this.getSite()
                                                                          .getShell(),
                                                             loadedPkgs,
                                                             pkgs,
                                                             pkgsdoc,
      ...
      loadPackageButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.LOAD_PACKAGE));
      loadPackageButton.setToolTipText("Load Installed Package");

      connectButton = new Action("Reconnect") {
         public void run() {
            if ( !swank.isConnected() ||
                     MessageDialog.openQuestion(ReplView.this.getSite()
                                                             .getShell(),
                                                "Reconnect",
                                                "Are you sure you want to restart your Lisp session?")) {
               this.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.DISCONNECTED));
               ...
               this.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.RECONNECT));
               loadPackageButton.setEnabled(swank.getManagePackages());

            }
         }
      };
      connectButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.RECONNECT));
      ...
      packageButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.DEFPACKAGE));
      ...
      pauseButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.THREAD_DEBUG));
      ...
      clearButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.CLEAR));
      clearButton.setToolTipText("Clear Repl and Reset Sash");

      showSwankFramesButton = new Action("Show Swank Frames",
                                         Action.AS_CHECK_BOX) {
         public void run() {
            State currState = currState();
            if (currState != null &&
                     currState.getClass().getName().contains("DebugState")) {
               IPreferenceStore prefs = LispPlugin.getDefault()
                                                  .getPreferenceStore();
      ...
      showSwankFramesButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.SORT_POSITION));
      ...
      stepButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.STEP));
      stepButton.setToolTipText("Step");

      runTestsButton = new TestAction(getSite().getShell());
      runTestsButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.RUN_TESTS));
   ...
   public void widgetSelected(SelectionEvent e) {
      if (e.widget.getData() != null) {
         ...
         try {
            LispPlugin.getDefault()
                      .getSwank()
                      .sendInspectInspectedPart(partNum,
   ...
   /**
    * This is a real class rather than an anonymous one so that it can be cloned
    * properly. If it weren't, you'd sometimes get results printed twice on the
    * repl
    */
   ...
   protected void registerSwankListeners() {
      ...
      swank.addDisconnectCallback(new SwankRunnable() {
         public void run() {
            if ( !swank.isConnected()) {
               connectButton.setImageDescriptor(CuspResources.getImageDescriptor(CuspResources.DISCONNECTED));
               IStatusLineManager slm = getViewSite().getActionBars()
                                                     .getStatusLineManager();
      ...
      swank.addDebugListener(new SwankRunnable() {
         public void run() {
            String key = result.get(1).value + ":" + result.get(2).value;
            System.out.println("*debug found: " + key + "->" +
                     debugInfos.get(key));
    ...
    * prints an appropriate commented message, and forces the repl to scroll to
    * the bottom.
    */
      ...
      public void verifyKey(VerifyEvent evt) {
         if (evt.keyCode == 'a') {
            if (evt.stateMask == SWT.CONTROL ||
                     (evt.stateMask == SWT.COMMAND && Platform.getOS()
                                                              .equals(Platform.OS_MACOSX))) {
   ...
   protected class CheckEvalListener implements VerifyKeyListener {
      public void verifyKey(VerifyEvent event) {
         ...
         if (inReadMode) {
            ...
            doit = (ctrl && (event.keyCode == '\r' || event.keyCode == '\n') &&
                     !in.getDocument().get().matches("\\s*") && LispUtil.doParensBalance(in.getDocument()));
         }
         if (doit) {
   ...
   protected class DebugState implements State, MouseListener, KeyListener,
            TreeListener, SelectionListener {
         ...
         if (condExtras.getNumberOfSubSexps() > 0) {
            for (int i = 0; i < condExtras.getNumberOfSubSexps(); i++ ) {
               ...
               if (extra.car().value.equals(":show-frame-source")) {
                  ...
                  try {
                     getSwank().sendGetFrameSourceLocation(frame,
                                                           thread,
                                                           new SwankRunnable() {
                                                              public void run() {
      ...
      public void chooseRestart(char c) {
         ...
         // distrs.
         /*
          * case 'a': swank.sendAbortDebug(null);
          * appendText("]> Abort debug\n"); break; case 'c':
          * swank.sendContinueDebug(null); appendText("]> Continue debug\n");
          * break;
          */
            case 's':
      ...
      public void treeExpanded(TreeEvent e) {
         ...
         if (sel.getData("frame") != null) {
            ...
            try {
               getSwank().sendGetFrameLocals(frame.toString(),
                                             thread,
                                             new SwankRunnable() {
                                                public void run() {
                                                   int n0 = sel.getItemCount();
                                                   // sel.removeAll(); //this
                                                   // precludes displaying on Linux
                                                   LispNode vars = result.getf(":return")
                                                                         .getf(":ok");
                                                   if (vars.params.size() <= 0) {
                                                      ...
                                                      for (int i = 0; i < vars.params.size(); ++i) {
                                                         ...
                                                         if (i < n0) {
                                                            ...
                                                            varItem = new TreeItem(sel,
                                                                                   0);
                                                         }
                                                         varItem.setText(name +
                                                                  " = " + val);
                                                         varItem.setData("frameNum",
                                                                         frame);
                                                         varItem.setData("varNum",
                                                                         i);
                                                      }
      ...
      public void widgetSelected(SelectionEvent e) {
         ...
         if (sel.getData("frame") != null) {
            ...
            try {
               getSwank().sendGetFrameSourceLocation(frame.toString(),
                                                     thread,
                                                     new SwankRunnable() {
                                                        public void run() {
                                                           processFrameSourceLocationResult(result);
      ...
      public void widgetDefaultSelected(SelectionEvent e) {
         ...
         if (item.getData() != null && item.getData() instanceof Integer) {
            ...
            if (item.getData("frameNum") != null &&
                     item.getData("varNum") != null) {
               Object frame = item.getData("frameNum");
               Object var = item.getData("varNum");
               try {
                  getSwank().sendInspectFrameLocal(thread,
                                                   frame.toString(),
                                                   var.toString(),
                                                   new InspectorRunnable());
               }
